...
async def _check_maintenance(user_id):
...
def _get_anonymous_admin_markup():
...
def AdminRightsCheck(mystic):
    async def wrapper(client, message):
        maintenance_msg = await _check_maintenance(message.from_user.id)
        if maintenance_msg:
        ...
        _ = await _get_language_strings(message.chat.id)

        if message.sender_chat:
            return await message.reply_text(
                _["general_3"], reply_markup=_get_anonymous_admin_markup()
            )
